...
class Enemy : public Creature {
      public:
          Enemy(int x, int y, int width, int height, int velX, int velY, BITMAP *buffer, BITMAP *picture, Player *playa);
          void move();
          void xVelocity();
          void yVelocity();
          void scroll(int x);
...
Enemy::Enemy(int x, int y, int width, int height, int velX, int velY, BITMAP *buffer, BITMAP *picture, Player *playa) 
...
void Enemy::displayName(){
     textout_ex(buf, font, "Enemah!", locationX, locationY-10, makecol(0, 0, 255), -1);
}
void Enemy::checkPlatform(int b2_x, int b2_y, int b2_w, int b2_h)
{
    if((locationY + hgt) < floorHeight && checkCollision(b2_x, b2_y, b2_w, b2_h)==0)
    {
    ...
    if (checkCollision(b2_x, b2_y, b2_w, b2_h)==1)
    {
         ...
         else if((locationY+hgt)>=b2_y && (locationY+hgt)<=(b2_y+(b2_h/2)))
         {
